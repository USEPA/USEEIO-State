...
It requires _useeior_ >= v1.5.1 which will be installed from github using the remotes library if _useeior_ doesn't exist already in your R environment.
...
Then we build a custom state EEIO model for Colorado using useeior and a [new custom model specification file](model_specs/COEEIOv1.0-s-JGV-20.yml).
All the raw files including the relevant StateIO and National totals of Employment and GHGs by sector, and Indicators (for GHGs to convert to kg CO2e) tables are retrieved from the EPA Data Commons and stored locally in your machine-defined `appdata` directory under _stateio_ and _flowsa_, and _lciaformatter_, respectively (each are EPA Tools for Industrial Ecology modeling).

If you've already built it once, it will load a stored RDS version from an output directory in this project.
