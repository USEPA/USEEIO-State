...
These results were generated using a USEEIO State model for `r state` along with state-specific territorial GHG inventories estimated by EPA.
...
Table \@ref(tab:cbetotal) shows the total consumption based emissions for `r state` and the for the rest of the United States (RoUS) in million metric tons. 
...
The CBEI results are contrasted with the state territorial emissions in Figure \@ref(fig:cbeterritorial). 

```{r cbeterritorial, echo=FALSE, fig.width=6.5, fig.height=4, fig.cap="Comparison of consumption-based and territorial GHG emissions." }
...
The consumption-based emissions are between `r round(min(cbe_divided_by_terr),1)`-`r round(max(cbe_divided_by_terr),1)` times larger than the territorial emissions over the time period.
...
Figure \@ref(fig:perdollar) shows `r state`'s consumption-based emissions per dollar spent.
This metric is also shown for the rest of the U.S. to provide where `r state`'s CBE per dollar are in context. These metric is shown using a constant dollar to remove the influence of inflation over this measure.
...
```
A CBE change can be driven by an change in the consumption of goods and services or in the embodied carbon intensity, measured in GHG emissions per dollar spent on goods and services, or by changes in both.
...
rownames(rel_d_w_per_d) <- c("Consumption", "GHGs Per Dollar")
colnames(rel_d_w_per_d) <- years
rel_d_w_per_d_fig <- lineChartFigure(as.matrix(rel_d_w_per_d), "% change")
...
\@ref(fig:relchangeconsumptionvintensity) compares the relative change in consumption in `r state` with the relative change in the embodied carbon intensity of the items consumed.


```{r perperson, echo=FALSE, fig.cap="CBE per state resident."}
...
per_person <- lineChartFigure(as.matrix(per_p), "tonnes CO2e per person")
per_person <- per_person + theme(text = element_text(size=12))
per_person
```

Consumption-based emissions intensity can also be measured in consumption-based emissions per resident. The CBE per `r state` resident is shown in Figure  \@ref(fig:perperson). This metric is also shown for the rest of the U.S. to provide where `r state`'s CBE per resident are in context.
...
Figure \@ref(fig:cbebysector) shows the CBE of `r state` by aggregate sector that the GHGs are associated with and the region of origin of the goods and services for year `r focal_year`.

```{r cbebysector, echo=FALSE, fig.cap="CBE by aggregate sector and good/service origin. RoUS = Rest of US; RoW = Rest of World", warning=FALSE}
...
Figure \@ref(fig:cbebysectorman) shows the CBE of `r state` for manufactured goods consumed and by region of origin of the goods for year `r focal_year`.
...
cbe_all_year_man <- subset(cbe_all_year, startsWith(Sector, "3") | startsWith(Sector, "23"))
...
Figure \@ref(fig:trade) shows the trend balance of emissions for the time series.
