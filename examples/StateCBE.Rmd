...
\\@ref(tab:cbe_total) shows the total consumption based emissions for `r state` and the for the rest of the United States (RoUS) in million metric tons. 
...
\@ref(fig:per_dollar) shows `r state`'s consumption-based emissions per dollar spent.
...
If the relative increase in consumption is outpacing the relative decrease in embodied carbon intensity, then consumption-based emissions will increase. 

```{r rel_change_consumption_v_intensity, fig.caption="Relative change in consumption in the state contrasted with relative change in consumption intensity, measured in GHGs per constant dollar", echo=FALSE}
...
Consumption-based emissions intensity can also be measured in consumption-based emissions per resident. The CBE per `r state` resident is shown in  \@ref(fig:per_person). This metric is also shown for the rest of the U.S. to provide where `r state`'s CBE per resident are in context.
...
\@ref(fig:cbe_by_sector) shows the CBE of `r state` by aggregate sector that the GHGs are associated with and the region of origin of the goods and services for year `r focal_year`.

```{r cbe_by_sector, echo=FALSE, fig.cap="CBE by aggregate sector and good/service origin. RoUS = Rest of US; RoW = Rest of World", warning=FALSE}
...
\@ref(fig:cbe_by_sector_man) shows the CBE of `r state` for manufactured goods consumed and by region of origin of the goods for year `r focal_year`.

```{r cbe_by_sector_man, echo=FALSE, fig.cap="CBE for manufactured goods consumed in the state. RoUS = Rest of US; RoW = Rest of World", warning=FALSE}
