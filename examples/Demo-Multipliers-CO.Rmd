...
It requires _useeior_ >= v1.5.1 which will be installed from github using the pak library if _useeior_ doesn't exist already in your R environment. The kableExtra package is used for formatting the tables for output here.
...
Then we build a custom state EEIO model for Colorado using _useeior_ and a [new custom model specification file](model_specs/COEEIOv1.0-JGV-20.yml).
...
```{r}
CO_result <- colSums(result$H_r[grep("US-CO",row.names(result$H_r)),])
RoUS_result <- colSums(result$H_r[grep("RoUS",row.names(result$H_r)),])
